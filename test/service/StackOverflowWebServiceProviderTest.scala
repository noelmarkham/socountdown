...
class StackOverflowWebServiceProviderTest extends Specification {
  "The date calculator" should {
    "return none for a too high score" in {
      val value = StackOverflowWebServiceProvider.projectedDate(2, 1, new Date())
      value match {
        case None => true
    ...
    "produce a date in the future for a real calculation" in {
      ...
      val value = StackOverflowWebServiceProvider.projectedDate(300000, 1000000, nowCal.getTime)
      value match {
